...
#ifdef CONFIG_SERIAL_UARTLITE_CONSOLE
		case UARTLITE:
			pr_debug("Early console on uartlite at 0x%08x\n",
								base_addr);
...
#ifdef CONFIG_SERIAL_8250_CONSOLE
		case UART16550:
			pr_debug("Early console on uart16650 at 0x%08x\n",
								base_addr);
			early_console = &early_serial_uart16550_console;
			break;
#endif
		default:
			pr_debug("Unsupported early console %d\n",
								version);
			return 1;
		}
...
void __init remap_early_printk(void)
{
	if (!early_console)
		return;
	pr_debug("early_printk_console remapping from 0x%x to ", base_addr);
