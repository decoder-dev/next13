...
static int aw8697_rtp_trim_lra_calibration(struct aw8697 *aw8697)
{
	...
	lra_rtim_code =
	    aw8697_clock_OSC_trim_calibration(theory_time, aw8697->microsecond);
	if (lra_rtim_code > 0) {
...
static int aw8697_rtp_osc_calibration(struct aw8697 *aw8697)
{
	...
	while (1) {
		...
		if (osc_int_state & AW8697_BIT_SYSINT_DONEI) {
			...
			pr_debug
			    ("%s vincent playback done aw8697->rtp_cnt= %d \n",
			     __func__, aw8697->rtp_cnt);
			break;
		}
		...
		if (aw8697->microsecond > OSC_CALIBRATION_T_LENGTH) {
			pr_debug
			    ("%s vincent time out aw8697->rtp_cnt %d osc_int_state %02x\n",
...
static void aw8697_rtp_work_routine(struct work_struct *work)
{
	...
	while (atomic_read(&aw8697->is_in_rtp_loop)) {
		...
		mutex_unlock(&aw8697->lock);
		ret =
		    wait_event_interruptible(aw8697->wait_q,
					     atomic_read(&aw8697->
							 is_in_rtp_loop) == 0);
		pr_debug("%s  wakeup \n", __func__);
		mutex_lock(&aw8697->lock);
...
static void aw8697_vibrator_work_routine(struct work_struct *work)
{
	...
	pr_debug("%s enter\n", __func__);
	pr_debug
	    ("%s: effect_id = %d state=%d activate_mode = %d duration = %d\n",
	     __func__, aw8697->effect_id, aw8697->state, aw8697->activate_mode,
	     aw8697->duration);
	mutex_lock(&aw8697->lock);
			   ...
			   struct device_node *np)
{
	...
	for (i = 0; i < 3; i++)
		for (j = 0; j < 5; j++)
			printk
			    (" 20190420_dt       aw8697->info.trig_config[%d][%d]: %d\n",
					...
					struct ff_effect *old)
{
	...
	while (atomic_read(&aw8697->exit_in_rtp_loop)) {
		...
		mutex_unlock(&aw8697->lock);
		ret =
		    wait_event_interruptible(aw8697->stop_wait_q,
					     atomic_read(&aw8697->
							 exit_in_rtp_loop) ==
					     0);
		pr_debug("%s  wakeup \n", __func__);
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (!aw8697->enable_pin_control) {
		if (gpio_is_valid(aw8697->reset_gpio)) {
			ret =
			    devm_gpio_request_one(&i2c->dev, aw8697->reset_gpio,
						  GPIOF_OUT_INIT_LOW,
						  "aw8697_rst");
			if (ret) {
