...
int write_rb(const char *data, int32_t size)
{
	...
	pr_debug("%s: write  write index %d, read index %d, free size %d",
	...
	if (atomic_read(&grb->exit) == 1) {
		pr_debug("%s: exit write_rb", __func__);
		return -EPERM;
	}
...
int read_rb(char *data, int32_t size)
{
	...
	pr_debug("%s: read_rb data:%p, size %d", __func__, data, (int)size);
	...
	if (size > filled_size)
		pr_debug("%s: buffer underrun , req size %d, filled size %d",
			 __func__, size, filled_size);
...
{
	pr_debug("%s: rb force exit", __func__);
