...
static void *vcu_check_gce_pa_base(struct mtk_vcu_queue *vcu_queue, u64 addr, u64 length)
{
	...
	pr_debug("%s addr %x length %x not found!\n", __func__, addr, length);
	...
	u64 addr, u64 data, u32 mask, u32 gpr, u32 dma_offset, u32 dma_size)
{
	...
	case CMD_READ:
		if (vcu_check_reg_base(vcu, addr, 4) == 0)
			...
			pr_debug("[VCU] CMD_READ wrong addr: 0x%x\n", addr);
	break;
	case CMD_WRITE:
		if (vcu_check_reg_base(vcu, addr, 4) == 0)
			cmdq_pkt_write(pkt, vcu->clt_base, addr, data, mask);
		else
			pr_debug("[VCU] CMD_WRITE wrong addr: 0x%x 0x%x 0x%x\n",
				addr, data, mask);
	break;
	...
	case CMD_POLL_REG:
		if (vcu_check_reg_base(vcu, addr, 4) == 0)
			...
			pr_debug("[VCU] CMD_POLL_REG wrong addr: 0x%x 0x%x 0x%x\n",
				addr, data, mask);
	break;
	case CMD_WAIT_EVENT:
	...
	case CMD_MEM_MV:
		if ((vcu_check_reg_base(vcu, addr, 4) == 0 ||
			...
			pr_debug("[VCU] CMD_MEM_MV wrong addr/data: 0x%x 0x%x\n",
				addr, data);
	break;
	case CMD_POLL_ADDR:
		if (vcu_check_reg_base(vcu, addr, 4) == 0 ||
...
void mtk_vcu_mem_release(struct mtk_vcu_queue *vcu_queue)
{
	...
	list_for_each_safe(p, q, &vcu_queue->pa_pages.list) {
		...
		cmdq_mbox_buf_free(
			...
			(dma_addr_t)tmp->pa);
		pr_debug("Free cmdq pa %lx ref_cnt = %d\n", tmp->pa,
