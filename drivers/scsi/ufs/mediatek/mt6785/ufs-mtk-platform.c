...
{
	unsigned long flags;
	enum clk_gating_state cg_state;

	spin_lock_irqsave(ufs_mtk_hba->host->host_lock, flags);
	cg_state = ufs_mtk_hba->clk_gating.state;
	spin_unlock_irqrestore(ufs_mtk_hba->host->host_lock, flags);

	/* Link state should be H8 if ref-clk is off */
	if (cg_state == CLKS_OFF)
		return UFS_H8;
	else
		return -1;
}
