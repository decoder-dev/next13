...
static int l2tp_mt_check(const struct xt_mtchk_param *par)
{
	...
	if ((!(info->flags & XT_L2TP_TID)) &&
		...
		pr_debug("invalid flags combination: %x\n", info->flags);
		return -EINVAL;
	}
	...
	if (info->flags & XT_L2TP_VERSION) {
		...
		if (info->version == 2) {
			if ((info->flags & XT_L2TP_TID) &&
			    (info->tid > 0xffff)) {
				pr_debug("v2 tid > 0xffff: %u\n", info->tid);
				return -EINVAL;
			}
			if ((info->flags & XT_L2TP_SID) &&
			    (info->sid > 0xffff)) {
				pr_debug("v2 sid > 0xffff: %u\n", info->sid);
				return -EINVAL;
			}
		}
	}
...
static int l2tp_mt_check4(const struct xt_mtchk_param *par)
{
	...
	if ((ip->proto != IPPROTO_UDP) &&
		...
		pr_debug("missing protocol rule (udp|l2tpip)\n");
		return -EINVAL;
	}

	if ((ip->proto == IPPROTO_L2TP) &&
	    (info->version == 2)) {
		pr_debug("v2 doesn't support IP mode\n");
		return -EINVAL;
	}
